...
test.describe.serial('dashboard summary flow', () => {
  ...
  test('after logging an entry, dashboard shows latest entry and count 1', async ({ page }) => {
    ...
    await page.goto('/');
    // Scope to the Latest Entry heading: the trend chart's visually-hidden
    // accessible data table also contains this weight value in a <td>, so a
    // bare getByText is ambiguous under Playwright strict mode.
    await expect(page.getByRole('heading', { name: '175.5 lbs' })).toBeVisible({ timeout: 5_000 });
